...
test('full pipeline with self-healing: prompt → KiCad files', async () => {
  ...
  const { circuitJson, kicadFiles, validation } = result.data ?? {}
...
test('pipeline handles complex circuit requirements', async () => {
  ...
  const { circuitJson, kicadFiles } = result.data ?? {}

  expect(circuitJson.length).toBeGreaterThan(2)

  const _hasRegulator = circuitJson.some((el: any) => el.name?.startsWith('U'))
  const hasResistor = circuitJson.some((el: any) => el.name?.startsWith('R'))
  const _hasCapacitor = circuitJson.some((el: any) => el.name?.startsWith('C'))
  const _hasLED = circuitJson.some((el: any) => el.name?.startsWith('LED'))
...
test('pipeline returns structured errors on failure', async () => {
  const result = await runPipeline('', { runErc: true })
