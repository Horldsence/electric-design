...
test.skipIf(!kicadInstalled, 'circuit with unconnected pins - ERC should detect violations', async () => {
  const code = `
    export default () => (
...
test.skipIf(!kicadInstalled, 'circuit with clearance issues - DRC should detect violations', async () => {
  const code = `
    export default () => (
